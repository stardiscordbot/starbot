module.exports = class DailyCommand {
    ...
    async run(ctx) {
        if(!ctx.args[0]) return ctx.message.channel.createMessage(`:x: ${ctx.message.author.mention} **|** Insira um quantia **válida** para depositar.`)

        const money = await db.get(`money-${ctx.message.author.id}`) || 0;
        const banco = await db.get(`banco-${ctx.message.author.id}`) || 0;

        if(money == 0 || money < 0) return ctx.message.channel.createMessage(`:x: ${ctx.message.author.mention} **|** Você não dinheiro em sua carteira.`)
        ...
        if(ctx.args[0] == "all" || ctx.args[0] == "tudo") {
            ...
            ctx.message.channel.createMessage(`:white_check_mark: ${ctx.message.author.mention} **|** Você depositou **¥ ${money.toLocaleString()}** em seu banco.`)
        } else {
        ...
        ctx.message.channel.createMessage(`:white_check_mark: ${ctx.message.author.mention} **|** Você depositou **¥ ${ctx.args[0].toLocaleString()}** em seu banco.`)
        }
    }
}