module.exports = {
    ...
    getMemberAll: function(client, toFind = '') {
        toFind = toFind.toLowerCase();
        let target = client.guilds.cache.map(g => g.members.cache.get(toFind));

        if (!target && toFind) {
            target = client.guilds.cache.map(g=> g.members.cache.find(member => {
                return member.displayName.toLowerCase().includes(toFind) ||
                member.user.tag.toLowerCase().includes(toFind)
            }));
        }
