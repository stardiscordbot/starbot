...
class Util {
	...
	static getVideoBySearch(search, ytsr, options = {}) {
		return new Promise(async (resolve, reject) => {
			...
			if (isVideoLink) {
				...
				ytsr(filters.query, searchOptions)
					.then((searchResults) => {
						let items = searchResults.items;

						if (!items || !items[0]) return reject('SearchIsNull');

						if (items[0].type != 'Video') items.shift();
	...
	static buildBar(value, maxValue, size, loadedIcon, arrowIcon) {
		...
		return `[${progressText}${emptyProgressText}][${this.MilisecondsToTime(
			value
		)}/${this.MilisecondsToTime(maxValue)}]`;
	}
}
