...
class Player {
	...
	constructor(client, options = {}) {
		if (!client)
			throw new SyntaxError('[Discord_Client_Invalid] Invalid Discord Client');
		if (typeof options != 'object')
			throw new SyntaxError(
				'[Options is not an Object] The Player constructor was updated in v5.0.2, please use: new Player(client, { options }) instead of new Player(client, token, { options })'
			);
		/**
		...
		client.on('voiceStateUpdate', (oldState, newState) => {
			...
			let queue = this.queues.find(
				(g) => g.connection.channel.id === oldState.channelID
			);
			if (queue) {
	...
	async play(voiceChannel, songName, options = {}, requestedBy) {
		...
		if (!voiceChannel || typeof voiceChannel !== 'object')
			return new MusicPlayerError('VoiceChannelTypeInvalid', 'song');
		if (typeof songName !== 'string')
			return new MusicPlayerError('SongTypeInvalid', 'song');
		if (typeof options != 'object')
			return new MusicPlayerError('OptionsTypeInvalid', 'song');
		try {
			// Searches the song
	...
	async addToQueue(guildID, songName, options = {}, requestedBy) {
		...
		if (typeof songName !== 'string')
			throw new MusicPlayerError('SongTypeInvalid', 'song');
		if (typeof options != 'object')
			throw new MusicPlayerError('OptionsTypeInvalid', 'song');
		try {
			// Searches the song
			let video = await Util.getVideoBySearch(songName, ytsr, options);
			// Define the song
	...
	async _playSong(guildID, firstPlay) {
		...
		if (!firstPlay)
			queue.emit(
				'songChanged',
				!queue.repeatMode ? queue.songs.shift() : queue.songs[0],
				queue.songs[0],
				queue.skipped,
				queue.repeatMode
			);
		...
		let dispatcher = queue.connection.play(
			ytdl(song.url, {
				filter: 'audioonly',
				quality: Quality,
				highWaterMark: 1 << 25,
