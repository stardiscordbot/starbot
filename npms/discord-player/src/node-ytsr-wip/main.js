...
const main = (module.exports = async (searchString, options, rt = 3) => {
	...
	// Parse items
	const rawItems = wrapper.contents.find(
	...
	resp.filters = Array.from(filters)
		.map((a) => a[1].active)
		.filter((a) => a);

	// Parse the nextpageToken
	const continuation = wrapper.contents.find(
		(x) => Object.keys(x)[0] === 'continuationItemRenderer'
	);
	let token = null;
	if (continuation)
		token =
			continuation.continuationItemRenderer.continuationEndpoint.continuationCommand
				.token;
