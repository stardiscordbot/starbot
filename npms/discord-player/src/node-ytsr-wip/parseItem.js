...
const parseVideo = (obj) => {
	...
	const authorThumbnails =
		obj.channelThumbnailSupportedRenderers.channelThumbnailWithLinkRenderer
	...
	return {
		...
		description: obj.descriptionSnippet
			? UTIL.parseText(obj.descriptionSnippet)
			: '',
		...
		uploaded_at: !obj.publishedTimeText
			? null
			: UTIL.parseText(obj.publishedTimeText),
...
const parseChannel = (obj) => {
	...
	return {
		...
		subscribers: obj.subscriberCountText
			? UTIL.parseNumFromText(obj.subscriberCountText)
			: null,
		// TODO: remove empty string when done caring about compatibility
		description_short: obj.descriptionSnippet
			? UTIL.parseText(obj.descriptionSnippet)
			: '',
		...
		followers: obj.subscriberCountText
			? UTIL.parseNumFromText(obj.subscriberCountText)
			: 0,
	};
};
...
const parsePlaylist = (obj) => {
	...
	return {
		...
		firstVideo:
			Array.isArray(obj.videos) && obj.videos.length > 0
				? {
					...
					bestThumbnail: obj.thumbnails[0].thumbnails.sort(
						(a, b) => b.width - a.width
					)[0],
				  }
				: null,
		...
		published_at: obj.publishedTimeText
			? UTIL.parseText(obj.publishedTimeText)
			: null,
		...
		thumbnail: obj.thumbnails[0].thumbnails.sort((a, b) => b.width - a.width)[0]
			.url,
		author: {
			name: owner.text,
...
const parseHorizontalCardListRenderer = (obj) => ({
	...
	entrys: obj.cards.map((c) => {
		const targetUrl =
			c.searchRefinementCardRenderer.searchEndpoint.commandMetadata.webCommandMetadata
				.url;
		return {
			q: UTIL.parseText(c.searchRefinementCardRenderer.query),
			link: URL.resolve(BASE_VIDEO_URL, targetUrl),
			bestThumbnail: c.searchRefinementCardRenderer.thumbnail.thumbnails.sort(
				(a, b) => b.width - a.width
			)[0],
			thumbnails: c.searchRefinementCardRenderer.thumbnail.thumbnails.sort(
				(a, b) => b.width - a.width
			),
		};
	}),
...
const parseMovie = (obj) => {
	...
	return {
		...
		author: {
			...
			link: URL.resolve(
				BASE_VIDEO_URL,
				owner.navigationEndpoint.browseEndpoint.canonicalBaseUrl || targetUrl
			),
			ownerBadges: (obj.ownerBadges || []).map(
				(a) => a.metadataBadgeRenderer.tooltip
			),
			verified: isOfficial || isVerified,
		},
		description: obj.descriptionSnippet
...
const parseShow = (obj) => {
	...
	return {
		...
		episodes: UTIL.parseNumFromText(
...
const parseShelf = (obj) => {
	const rawItems =
		obj.content.verticalListRenderer || obj.content.horizontalMovieListRenderer;
	return {
		type: 'shelf',
		title: UTIL.parseText(obj.title),
