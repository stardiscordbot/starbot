...
module.exports = (client) => {
	...
	client.ws.on('INTERACTION_CREATE', async (interaction) => {
		if(!interaction.member) {
			client.api.interactions(interaction.id, interaction.token).callback.post({
				data: {
					...
					data: {
						content:
							':x: **|** A Star ainda não suporta comandos na DM. Utilize comandos apenas em servidores :blush:',
					},
				},
			});
		}
		...
		msg.quote = async function(content, options) {
		...
		if (comando.permissoes.membro.length) {
			// var _perm = new Discord.Permissions(comando.permissoes.membro)
			if (!msg.member.permissions.has(comando.permissoes.membro)) {
				return msg.quote(
					`:x: ${membroInt} **|** Você não possui permissões necessárias para executar este comando. Você precisa das seguintes permissões: \`${comando.permissoes.membro
						.join(
							', ',
		...
		if (comando.permissoes.bot.length) {
			if (
				!msg.guild.me.permissions.has(comando.permissoes.bot)
			) {
				return msg.quote(
					`:x: ${
						msg.author
					}  **|** Eu não possuo permissões necessárias para executar este comando. Preciso das seguintes permissões: \`${comando.permissoes.bot
						.join(
							', ',
