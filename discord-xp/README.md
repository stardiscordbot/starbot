...
client.on("message", async (message) => {
  ...
  if (hasLeveledUp) {
    ...
    message.quote(`${message.author}, congratulations! You have leveled up to **${user.level}**. :tada:`);
  }
});
```
- **Rank Command**
...
if (!user) return message.quote("Seems like this user has not earned any xp so far."); // If there isnt such user in the database, we send a message in general.

message.quote(`> **${target.tag}** is currently level ${user.level}.`); // We show the level.
```
